...
- **루터(looter)**: `behavior=looter`인 몬스터(현재 놀)는 싸울 상대가 없을 때 바닥 아이템을 주우러 간다. 15m 안에서 가장 가까운 것을 골라 달려가고 1.5m 안에서 요청한다. 전투 분기 **아래**에 있으므로 타겟이 있으면 평소대로 싸운다
  - **줍기는 요청일 뿐이다.** AI는 소유자 클라이언트에서 돌므로 `ClientMessage::MonsterPickupItem`은 검증 없이 신뢰하지 않는다 — 서버가 소유권(`is_controllable_by`)·층 일치·거리(`MAX_PICKUP_DISTANCE`)·소지 상한을 모두 다시 본다. 거절은 조용하다(브레인에 에러 경로가 없다)
  - **소지 상한 4스택**(`MONSTER_LOOT_STACK_LIMIT`). 없으면 방치된 바닥 아이템이 한 마리에 몰려 사망 시 한꺼번에 쏟아진다. 브레인도 같은 수에서 요청을 멈추고, 요청 간 2초 쿨다운을 둬 거절이 매 프레임 재시도가 되지 않게 한다
  - 들고 있는 것은 **메모리 전용**(`monster_loot`)이다. 죽으면 시체 주변에, **디스폰해도 서 있던 자리에** 되돌려 놓는다 — 소유자가 로그아웃했다고 바닥에 있던 아이템이 사라지면 안 된다
