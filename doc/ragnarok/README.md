...
| 13 | [구현 방향서](13_IMPLEMENTATION_DIRECTION.md) | **채택 항목별 손댈 파일·스키마·프로토콜·마이그레이션·검증·성능** (고정 ID `IMP-x.y`) |

## 읽는 순서

- 처음이면 **09 → 10 → 13** 순서로 읽는다. 09는 무엇을, 10은 단계를, 13은 어느 파일에
  어떻게 붙는지를 정한다. **10과 13이 어긋나면 13이 정본이다** (13은 실제 코드를 읽고 쓴 것이라
  10의 전제를 몇 군데 정정한다).
- 착수 순서·게이트·마일스톤은 [DEVELOPMENT_MASTER_PLAN](../DEVELOPMENT_MASTER_PLAN.md),
  현재 시스템의 구성도는 [ARCHITECTURE](../ARCHITECTURE.md).
