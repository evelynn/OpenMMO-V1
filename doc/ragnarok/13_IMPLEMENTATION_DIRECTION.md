...
[10_IMPLEMENTATION_ROADMAP](10_IMPLEMENTATION_ROADMAP.md)이 *순서*를 정했다.
...
아래 28개 항목의 설계가 RO 원본과 갈라지는 지점은 거의 전부 이 셋 중 하나에서 나온다.
...
**검증**
- `inventory.rs` 테스트: 티어 1/3/5의 +5→+6 성공률이 각각 100/75/50%인지, 1% 바닥이
  모든 티어에서 유지되는지.
...
| IMP-3.2 | 전투 스킬 시스템 | 12 선결 / 28은 기각·아이디어만 차용 | 3.1 |
...
| 결정 | 채택 (09 #14) |
| 난이도 | 소 |
| 선행 조건 | 없음 (IMP-3.4와 같은 PR을 권장 — 두 항목 모두 가격 계산 지점을 건드린다) |
