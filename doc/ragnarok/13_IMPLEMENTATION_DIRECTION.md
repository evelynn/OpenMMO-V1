...
### IMP-6.1. 도시 서비스 NPC 배치

| | |
|---|---|
| 결정 | 채택 (베타 게이트 — IMP-2.2/2.3/2.4가 미뤄 둔 배치) |
| 난이도 | 중 |
| 선행 조건 | **지형 베이크** (이제 충족) |
| 프로토콜 변경 | 없음 |

**구현 방향**
IMP-2.2(세이브 포인트)·IMP-2.3(창고)·IMP-2.4(유료 이동)는 세 번에 걸쳐 이 배치를
미뤘고 이유는 매번 같았다: **검증된 월드 좌표가 없다.** 지형이 없으면 좌표가 벽
안인지 허공인지 확인할 방법이 없고, 확인하지 않은 배치는 나중에 NPC가 지형에
박히는 형태로 드러난다. 이 환경에는 스폰 리전을 포함해 25리전이 구워져 있으므로
**이제 확인할 수 있다.**

**좌표는 눈대중이 아니라 샘플링으로 골랐다.** 스폰(-1475.2, 4741.6) 둘레를 8m·12m
링으로 돌며 `HeightSampler`와 `WaterSampler`로 지면 높이·수면·기울기를 재고,
**평평하고(기울기 0.05 이하) 마른 땅**을 골랐다. 자리는
**(-1479.2, 0.69, 4748.5)** — 스폰에서 8m, 리카에게서 16m.

**세 기능의 게이트는 바꾸지 않는다.** 여전히 **공식 NPC 누구나** 열어 준다 —
좁히면 미라가 접속하지 않은 동안 창고를 못 여는 서버가 된다. 바뀌는 것은
**안내 문구**뿐이다(설계가 "안내 문구를 그 NPC로 좁힌다"고 한 그대로).

> **개정 (IMP-6.1 착수 시)** — 두 가지.
>
> 1. **스폰이 크립트 발자국 안에 있는 것은 장애물이 아니다.** 처음에는 자리를
>    `old_crypt`의 80×80 발자국 밖으로 잡으려 했는데, 그 발자국은 **"이 클라이언트가
>    지하인가"를 판정할 때만** 쓰이고 판정 자체는 `floor_level`이 한다. 리카도
>    발자국 안에 서 있고 잘 돈다. 발자국 밖을 고집하면 스폰에서 16m 떨어진
>    해안 쪽 낮은 땅으로 밀려나므로, 조건에서 뺐다.
> 2. **플레이어가 지은 집은 확인하지 못했다.** 개발 상태 디렉터리에는 집이 없어서
>    이 좌표가 **빈 땅이라는 것까지만** 검증했다. 집이 있는 배포 월드에서는
>    `housing/`과 겹치는지 한 번 더 봐야 한다.

**검증** — 서버가 5개 NPC 정의를 읽고 부팅 assert를 통과한다. 좌표는 위의 샘플링
결과가 근거다.

---

## Phase 7 — 수용 인원과 인구 (유지보수자 결정, 2026-08-20)

두 결정이 같은 날 내려왔고 서로 맞물린다. **동시 접속 목표를 1,000명으로 낮추고
채널로 분할한다**(IMP-6.3이 5,000의 유지 불가를 실측한 직후다), 그리고
**솔로 플레이가 성립하도록 파티 가능한 월드 봇을 넣는다.**
...
| | |
|---|---|
| 결정 | 채택 (유지보수자, 2026-08-20) |
| 난이도 | 중~대 |
| 선행 조건 | IMP-6.3 (수용 한계 실측) |
| 프로토콜 변경 | 있음 — `SwitchChannel` → `ChannelState` |
| 저장 스키마 변경 | **없음 — DB는 채널이 나누지 않는다** |

**결정된 모양**: **같은 DB · 월드만 분리.** 캐릭터·인벤토리·창고·길드·우편·친구는
채널과 무관하게 하나다. 채널이 나누는 것은 **누가 누구를 보는가**뿐이고, 채널을
바꿔도 같은 캐릭터로 논다.

**손댈 파일**
- `shared/src/channel.rs`(신규) — `ChannelId`, `CHANNEL_CAPACITY = 1_000`.
- `shared/src/messages.rs` — `SwitchChannel` / `ChannelState`.
- `server/src/game_state/channel.rs`(신규) — 배정·이동·정원.
- `server/src/game_state/player.rs:2379` `players_within_position` — **여기 한 곳**.
- `client/src/lib/stores/channelStore.ts`(신규) + HUD 표시.

**구현 방향**
**채널은 `Player`에 넣지 않는다.** `Player`는 msgpack fixarray 상한인 15필드로 꽉
차 있고, 16번째는 모든 스냅숏의 모든 플레이어를 2바이트씩 키운다(§0의 빚).
그리고 **넣을 이유가 없다** — AOI가 이미 다른 채널을 걸러내므로 클라이언트는 남의
채널 번호를 알 필요가 없다. 서버가 `player_channels` 맵으로만 들고, 자기 채널은
`ChannelState`로 따로 받는다.

**거르는 지점은 한 곳이다.** 위치 기반 전달은 전부
`players_within_position`으로 모이므로(플레이어 가시성, 몬스터 이벤트, 바닥
아이템, 모닥불, 근처 채팅) 거기서 채널을 비교하면 나머지가 따라온다. 던전
인스턴스를 오버레이로 처리한 것(IMP-4.2)과 같은 방법이다.

**몬스터는 채널을 따로 갖지 않는다.** 지상 몬스터는 플레이어를 따라다니며 스폰되고
소유자가 있으므로 **몬스터의 채널 = 소유자의 채널**로 공짜로 정해진다. `Monster`도
와이어 타입이라 필드를 늘리지 않아도 되는 것이 이득이다.

**채널을 넘는 것과 넘지 않는 것**:

| 넘는다 (플레이어 지정 · DB) | 넘지 않는다 (AOI) |
|---|---|
| 귓속말, 파티 채팅, 길드 채팅, 우편 | 근처 채팅, 플레이어 가시성 |
| 창고, 인벤토리, 업적, 친구 목록 | 몬스터, 바닥 아이템, 모닥불·노점 |
| 파티·길드 명단 | 전투 이벤트 |

파티원이 서로 다른 채널에 있으면 **같이 싸울 수 없다** — 그래서 `SwitchChannel`은
정원이 남아 있는 한 언제든 되어야 하고, 파티창에 상대 채널이 보여야 한다.

**정원**: 채널당 1,000명(IMP-6.3 실측). 가득 차면 전환을 거절한다. 배정은 입장 시
**가장 한산한 채널**이고, 파티원이 있으면 그쪽을 먼저 시도한다.

**검증**
- `game_state` 테스트: 다른 채널의 플레이어가 서로 보이지 않는가, 귓속말은 넘는가,
  정원이 찬 채널로 전환이 거절되는가, 채널을 바꿔도 인벤토리·창고가 같은가.
- `tools/spike-socket-load.sh`: 채널 2개에 1,000명씩.

> **개정 (IMP-7.1 착수 시)** — 두 가지.
>
> 1. **채널 맵 락은 스윕당 한 번만 잡는다.** 후보마다 `channel_of`를 부르면
>    **질의 1회당 플레이어 수만큼** 락을 잡게 되고, 한 셀에 1,000명이면 그것은
>    조회가 아니라 경합이다(마스터 플랜 D6 #3). 루프 밖에서 한 번 잡아 클로저로
>    쓴다.
> 2. **실측: 1,200명이 4채널에 300씩 고르게 갔다.** `load-client` 1,200개를 붙였을
>    때 서버 관측치가 `players_online 1200`, 채널별 300/300/300/300,
>    `overflowed_connections 0`이었다. 밀집 틱 게이트(200명)도 회귀 없이
>    10.95ms/200ms다.

---

### IMP-7.2. 플레이어 봇 (파티 가능한 월드 봇)
...
**결정된 모양**: **실제 유저처럼 사냥터에서 사냥하고, 파티에 들어오면 파티도 되는
봇.** 솔로 플레이어가 파티 콘텐츠를 돌 수 있게 하는 것이 목적이고, 동시에 베타
초기의 빈 월드를 채운다.

**손댈 파일**
- `agent-client/src/driver/rule_bot.rs`(신규) — LLM 없는 규칙 드라이버.
- `tools/load-client/` — 접속·입장 골격을 이미 갖고 있다(IMP-6.3).

**구현 방향**
**새 클라이언트를 만들지 않는다.** `agent-client`가 이미 월드 상태, 이동,
길찾기, 몬스터 추적, 파티 이벤트를 전부 들고 있고 `LlmType::None` 경로도 있다.
필요한 것은 **LLM 대신 규칙이 액션을 고르는 드라이버** 하나다.

**서버는 봇을 알지 못한다.** 봇은 `ClientMessage`만 쓰고 NPC 토큰으로 붙는다 —
사람이 하는 것과 서버에서 구별되지 않는다(제약 (c)). 봇 전용 메시지·필드·우회로를
만들지 않는다.

**행동 규칙** (우선순위 순, 위가 이긴다):
1. 체력이 낮으면 물러나 회복한다.
2. 파티에 속해 있으면 **리더를 따라간다** — 리더의 사냥감을 같이 친다.
3. 혼자면 자기 사냥터 반경 안에서 레벨에 맞는 몬스터를 고른다
   (IMP-1.1의 레벨 차 감쇠를 그대로 쓴다 — 봇이 저레벨 몹을 파밍하면 안 되는
   이유가 사람과 같다).
4. 파티 초대는 받는다. 거절 규칙은 두지 않는다 — 솔로 플레이어를 돕는 것이
   존재 이유다.
5. 할 일이 없으면 사냥터 안을 돈다.

**LLM 비용은 0이다.** 규칙 드라이버는 `llm_scheduler`를 타지 않는다. 이것이
IMP-4.5(에이전트 동반자)와 다른 점이다 — 그쪽은 인격이 있는 NPC를 고용하는 것이고,
이쪽은 **인구**다.

**검증**
- 봇이 몬스터를 실제로 잡는가(서버 로그의 처치 이벤트).
- 파티 초대 → 수락 → 리더 추적이 도는가.
- 봇 N명이 붙어 있을 때 채널 정원과 틱 예산이 유지되는가.

> **개정 (IMP-7.2 착수 시)** — 세 가지.
>
> 1. **봇 주위에는 사람이 없으면 몬스터가 안 나온다 — 그리고 그게 맞다.**
>    `tick_monster_spawns`는 **NPC 플레이어에게는 사람이 시야 안에 있을 때만**
>    앰비언트 스폰을 붙인다("보는 사람이 없는 에이전트 주위에 몬스터를 뿌릴 이유가
>    없다"). 봇은 NPC 토큰으로 붙으므로 이 규칙을 그대로 받는다. 처음에는 버그로
>    보였으나 **원하는 동작이다**: 솔로 플레이어가 봇을 데리고 가면 그 사람이
>    조건을 만족시키므로 사냥이 시작되고, 아무도 없을 때는 어차피 볼 사람이 없다.
>    비용도 그만큼 아낀다. 대신 **빈 서버에서 봇만으로는 전투를 관측할 수 없어서**
>    조준 규칙은 통합 실행이 아니라 단위 테스트로 고정했다.
> 2. **`LlmBackend`로 만든다.** 두 번째 클라이언트를 만들지 않는다 — 액션 파서,
>    이동·접근, 파티 처리를 전부 그대로 쓰고 **누가 액션을 고르는가만** 바뀐다.
>    덕분에 봇이 사람의 에이전트가 못 하는 행동에 손댈 수 없다는 것이 구조로
>    보장된다(테스트가 모든 결정이 파서를 통과하는지 확인한다).
> 3. **조준 규칙은 `SharedState`에서 떼어냈다.** `pick_target`이 몬스터 목록만
>    받도록 해서 규칙 자체를 단위 테스트한다 — 값진 부분은 조회가 아니라 **선택**이다.

**실행 방법** — `data/config.toml`의 NPC 항목에 `llm = "bot"`, `character_name`,
그리고 사냥터를 정할 `schedule_file`(첫 항목의 좌표가 자리다)을 준다. 첫 항목이
없으면 월드 스폰이 자리가 된다.

---

## Phase 8 — 콘텐츠 확충 (유지보수자 결정, 2026-08-20)

IMP-6.4를 두 항목으로 쪼갠다. 원래 한 줄("콘텐츠 확충, L, 제품 판단")이었는데,
그 안에 성격이 다른 두 가지가 섞여 있었다 — **놀 대상이 없는 것**(몬스터·퀘스트)과
**성장 축이 하나뿐인 것**(전직)이다. 앞은 데이터, 뒤는 프로토콜이라 함께 두면
어느 쪽도 끝나지 않는다.
...
| 결정 | 채택 (IMP-6.4에서 분리) |
| 난이도 | 중 |
| 프로토콜 변경 | **v50** — `QuestOffer.monster_id` → `target` |

**손댈 파일** — `data-src/monsters.csv`, `data-src/hunting_quests.csv`,
`data-src/world.json`, `data-src/world_bosses.csv`, `server/src/monster_defs.rs`,
`server/src/quest_defs.rs`, `server/src/game_state/quest.rs`,
`shared/src/messages.rs`, `client/src/lib/data/monsterDefs.ts`,
`client/src/lib/stores/questStore.ts`, `client/src/lib/components/QuestBoardPanel.svelte`.

**구현 방향**
**새 메시를 만들지 않는다.** `orc_boss`·`goblin_boss`·`ogre_boss`가 이미 base 모델 +
`scale`이다. 레벨 11~20의 12종을 같은 수법으로 만든다 — 캐릭터 메시 1종은 원화 →
3D 생성 → Mixamo 리깅 → Blender 정리까지 사람의 하루치이고, 이쪽은 CSV 한 줄이다.
13종 → **25종, 레벨 1~20**.

**`scale`을 올리면 이동 속도를 같은 배율로 올린다.** 공유 애니 팩의 보폭은 Hips 본의
지면 위 높이에 비례하고 `scale`이 그 높이를 곱하기 때문이다
(`doc/assets/monsters.md`). 안 올리면 발이 미끄러진다 — **기존 보스 3종이 실제로 그
상태였고 이번에 함께 고쳤다**(`goblin_boss`가 scale 1.4에 walk 0.8이었다).
단 **달리기는 8.0에서 자른다**: 스토커(8.0)보다 빠른 몬스터를 만들지 않는다는 쪽이
미끄러짐보다 중요하다. 상한에 걸린 셋(deep_stalker·troll_elder·troll_king)만
달릴 때 약간 미끄러진다.

**종족 축**(09 #25 재판정)은 라벨로 끝내지 않는다. `race`는 **사냥 퀘스트의 목표**가
되고, 부팅이 **비어 있는 종족을 가리키는 계약을 거절한다** — 09가 이 축을 기각한
이유가 정확히 "칸이 빈다"였으므로, 그 상태를 만들 수 없게 하는 것이 이 축의
전제 조건이다. 5종(`goblinoid` 9 · `giant` 6 · `orc` 5 · `beast` 3 ·
`aberration` 2)만 만든다.

`hunting_quests.csv`는 `monsterId`와 `targetRace` 중 **정확히 하나**를 갖는다. 둘 다
있거나 둘 다 없으면 부팅이 죽는다. 계약 6개 → **20개**.

> **레벨 20까지 몹을 넣어도 아크가 20까지 열리지는 않는다.** 캐릭터 XP가 누적
> `20·2^(n-2)`라 19→20 한 칸이 2,621,440이고 레벨 19 몹은 약 380을 준다 — 6,900마리다.
> 실질 아크는 **레벨 16 안팎**이고 17~20은 롱테일이다. 이 벽을 실제로 미는 것은
> IMP-8.1의 두 번째 축(Job)이지 몬스터 레벨이 아니다. 베이스 곡선 자체를 바꾸는
> 것은 기존 캐릭터 전원의 레벨을 다시 매기는 일이라 **별도 판단으로 남긴다.**

**월드 보스 2기의 좌표는 눈대중이 아니라 샘플링이다** — IMP-6.1과 같은 방법으로
베이크된 높이·수면·경사를 읽어 반경 6m가 평평하고 물이 1m 아래인 지점을 골랐다.
`gnoll_matriarch`는 스폰에서 460m(-1647.5, 4.81, 5168.1), `troll_king`은
980m(-554.3, 6.75, 4406.4)다.

**검증** — 부팅이 25종·20계약을 전부 교차 검증한다(모르는 몬스터, 빈 종족,
목표 0개, 뒤집힌 레벨 대역). 종족 계약이 여러 몬스터를 세고 다른 종족은 세지
않는지는 단위 테스트 1건.

> **개정 (착수 직후) — `monster_attack_clips.json`은 모델이 아니라 몬스터 id로
> 키가 잡힌다.** 처음 커밋에서 12종을 빠뜨렸다. 빠지면 `attack_clip_ms`가 0을
>돌려주고 `swing_commit_ms = 0`이 되어, **대상이 한 발 물러서는 순간 스윙이 끊긴다**
> — 조용히 어색해지는 종류의 버그다.
>
> 값 자체는 재는 것이 아니라 **알 수 있다**: 변종은 base 행을 그대로 복사해
> `model`과 `animAttack`이 같으므로 클립 길이가 정의상 동일하다(`scale`은 시간을
> 바꾸지 않는다). 12종 모두 (model, animAttack)이 일치하는 base가 유일했다.
> 파일은 도구가 쓰는 것과 **바이트 단위로 같은** 형식으로 넣었다.
>
> 재발을 막는 것이 진짜 수정이다 — **부팅이 거절한다**: `animAttack`이 있는데
> 측정값이 없으면 실행할 도구 이름과 함께 죽는다. 몬스터를 추가하면서 측정 도구를
> 다시 돌리지 않는 것은 쉽고 조용하기 때문에, 조용하지 않게 만들었다.
...
| 25 | 종족 축 10종 | ~~만들지 않는다~~ → **IMP-8.2에서 5종으로 채택**(2026-08-20). 기각 사유("13종에 축을 얹으면 칸이 빈다")가 25종이 되면서 소멸했다. 10종이 아니라 채워지는 5종만 만들고, 빈 종족을 가리키는 계약은 부팅이 거절한다 |
...
| 28 | 전직 트리 / 전생 / 3·4차 | ~~차용만~~ → **전직 2단은 IMP-8.1에서 채택**(2026-08-20). 게이트를 Base가 아니라 **Job 레벨**에 걸면 기각 사유였던 레벨 대역 불일치가 사라진다. **전생·3차·4차는 그대로 기각** |
