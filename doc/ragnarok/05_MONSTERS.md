...
> **개정 (2026-08-20) — 종족 축을 채택한다. 기각 사유가 소멸했다.**
> 기각 근거는 "몬스터가 13종뿐이라 축을 하나 더 얹으면 각 칸이 빈다"였다.
> IMP-8.2가 **25종·레벨 1~20**으로 늘리면서 그 근거가 사라졌다. 다만 RO의 10종을
> 그대로 가져오지 않는다 — **실제로 채워지는 5종만** 만든다:
>
> | race | 지금 채우는 몬스터 |
> |---|---|
> | `goblinoid` | 코볼드·고블린·홉고블린·벅베어 계열 (7) |
> | `orc` | 오크 계열 (4) |
> | `beast` | 놀 계열 (3) |
> | `giant` | 오거·트롤 계열 (6) |
> | `aberration` | 스토커 계열 (2) |
>
> **축이 하는 일이 없으면 넣지 않는다.** 라벨만 다는 것은 기각당한 상태와 같으므로,
> 종족은 **사냥 퀘스트의 목표**가 된다 — `hunting_quests.csv`가 `monsterId` 대신
> `targetRace`를 쓸 수 있다. "고블린 10마리"는 한 몹을 찍어 주는 안내지만
> "고블린류 20마리"는 **사냥터를 고르게 하는 목표**다. 무기의 종족 특효(RO의
> 종족 카드에 해당)는 후보로 남기고 지금은 넣지 않는다 — 09 #26이 카드/슬롯을
> 기각했으므로 특효를 얹을 그릇이 아직 없다.

> **개정 (2026-08-20) — 상위 티어는 새 메시가 아니라 기존 메시의 재사용이다.**
> `orc_boss`·`goblin_boss`·`ogre_boss`가 이미 **base 모델 + `scale`**로 만들어져
> 있다. 레벨 11~20 구간은 같은 수법을 쓴다. 새 캐릭터 메시 1종은 원화→3D→Mixamo
> 리깅→Blender 정리까지 사람의 하루치 작업인데, `scale`+스탯 변형은 CSV 한 줄이다.
>
> **다만 `scale`을 올리면 이동 속도도 같은 배율로 올려야 한다.** `doc/assets/monsters.md`가
> 적어 둔 대로 공유 애니 팩의 보폭은 **Hips 본의 지면 위 높이**에 비례하는데,
> `scale`은 그 높이를 그대로 곱한다. 배율을 안 올리면 발이 미끄러진다 — 기존
> 보스 3종이 실제로 그 상태였고(예: `goblin_boss` scale 1.4에 walk 0.8) 이번에 함께
> 고쳤다.
>
> **레벨을 20까지 올려도 아크가 20까지 열리지는 않는다.** 캐릭터 XP는 누적
> `20·2^(n-2)`라 레벨 19→20 한 칸에 **2,621,440 XP**가 필요하고 레벨 19 몹은
> 한 마리에 약 380을 준다 — 약 6,900마리다. 실질 아크는 **레벨 16 안팎**에서
> 끝나고 17~20은 의도적인 롱테일이다. 이 벽을 실제로 미는 것은 몬스터 레벨이 아니라
> **두 번째 성장 축**이다: Job XP는 `100·n(n+1)(2n+1)/6`이라 다항이고, 베이스 곡선이
> 멈추는 구간에서도 계속 지급된다([01_PROGRESSION](01_PROGRESSION.md) 개정, IMP-8.1).
> 베이스 곡선 자체를 바꾸는 것은 기존 캐릭터 전원의 레벨을 다시 매기는 일이라
> 별도 판단으로 남긴다.
