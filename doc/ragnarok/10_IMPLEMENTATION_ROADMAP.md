...
> **⚠ 이 문서보다 13이 우선한다.** [13_IMPLEMENTATION_DIRECTION](13_IMPLEMENTATION_DIRECTION.md)은
> 실제 코드를 읽고 쓴 문서이고, 아래 몇 곳의 전제를 정정한다 — §1.1(`monster_xp` 시그니처
> 유지), §2.1의 창고 무게, §2.2(퀘스트 스키마·저장 위치), §2.3(존 파일 `monsterSpawns`는
> 서버가 읽지 않는다 → CSV 경로), §3.2(`guard` 재정의 대신 별도 방어 축), §3.4(노점이 아니라
> NPC 거래 경로), §7(착수 순서에 우편함 추가). 충돌하면 13을 따른다.
> 착수 순서·게이트는 [DEVELOPMENT_MASTER_PLAN](../DEVELOPMENT_MASTER_PLAN.md).
...
> ▶ 13 IMP-1.1이 정정: `monster_xp` 시그니처는 **바꾸지 않고** `level_diff_mult_bp`를 따로 둔다.
...
- 서버 검증: 거리(도시 NPC 근처), 슬롯 상한, 원자성(중복 생성 방지). 기존 거래
  (`game_state/trading.rs`)의 원자성 패턴을 재사용한다.
  ▶ 13 IMP-2.3이 정정: **창고 자체에는 무게 제한이 없다** — 출금 시 인벤토리 하중만 검사한다.
...
> ▶ 13 IMP-2.5가 정정: CSV는 11컬럼(`boardId`/`rewardItem` 포함), 진행도는 캐릭터 레코드가 아니라 `character_quests` 테이블, 보상은 우편 지급.
...
> ▶ 13 IMP-2.7이 정정: 존 파일의 `monsterSpawns`를 **서버가 읽지 않으므로** 존 필드가 아니라 `data-src/world_bosses.csv` + 30초 틱으로 간다.
...
> ▶ 13 IMP-3.3이 정정: `guard`는 명중 판정(AC)으로 **유지**하고 `armorPct`/`armorFlat` 축을 새로 만든다.
...
> ▶ 13 IMP-3.5가 정정: `shared/src/stall.rs`의 노점은 물건을 담지 않는다. 수수료는 `trading.rs`의 판매 경로(상대가 상인이 아닐 때)에 건다.
...
> ▶ 13이 정정: 보상 경로(우편함, IMP-2.1)를 헌팅 보드보다 먼저 넣는다. 순서는 IMP-1.1 → IMP-2.1 → IMP-2.5 + IMP-2.6.
