...
**`guard`는 "맞느냐"를, `armor`는 "얼마나 아프냐"를 담당한다.** 서로 다른 축이고,
서로를 대체하지 않는다.

| 축 | 무엇을 정하는가 | 어디에 쓰이는가 |
|----|-----------------|------------------|
| `guard` (AC) | 공격이 **명중하는지** | `resolves_as_hit` — 공격자의 d20 + 보정이 이 수를 넘어야 한다 |
| `armorPct` (Hard) | 명중한 피해의 **비율** 감소 | 합산, **상한 60%** |
| `armorFlat` (Soft) | 명중한 피해의 **절대** 감산 | 합산, 상한 없음 |

doc 10 §3.2는 "`guard`의 의미를 재정의하고 환산표를 문서화한다"고 했지만 그러지
않았다. `guard`는 이미 명중 판정의 목표수로 코드 전체에 박혀 있고, 거기에 감산
의미를 겹치면 **한 숫자가 두 일을 하게 된다** — 환산표가 필요해지는 이유가 바로 그
겹침이다. 축을 하나 더 만드는 쪽이 싸고, D&D의 AC와 RO의 DEF가 원래 다른 축이므로
원본에도 더 충실하다.

### 적용 순서 (순서가 곧 스펙이다)

```
after_hard = damage × (100 − min(armorPct, 60)) / 100
final      = max(1, after_hard − armorFlat)
```

**비율 먼저, 감산 나중, 최소 1.** 뒤집으면 결과가 달라진다 — 감산을 먼저 하면
평평한 방어구가 **큰 공격일수록 더 값어치 있게** 되어 앞뒤가 맞지 않는다.

두 축을 함께 쓰는 이유는 각자의 실패 모드가 반대이기 때문이다.

- **비율만 있으면** 수치가 충분히 커지는 순간 무적이 된다 → 그래서 상한 60%.
- **감산만 있으면** 다단·저피해 공격이 전부 0이 된다 → 그래서 최소 1.

### 인챈트

방어구 인챈트(`+N`)는 **`guard`에만** 붙는다. `armor`에도 붙이면 한 번의 강화가 두
축을 동시에 올려 인챈트 사다리의 밸런스 기준이 사라진다.

### 데이터

`data-src/items.csv`의 `armorPct` / `armorFlat`. 빈 칸이 중립(0)이므로 채우지 않은
아이템은 아무 것도 바꾸지 않는다. 현재 최고 장비 한 벌(플레이트 + 레이븐 실드)이
**19%**로, 상한 60%까지는 여유가 있다.

- 구현: [server/src/game/combat.rs](../server/src/game/combat.rs) `apply_defense`,
  [server/src/game_state/combat.rs](../server/src/game_state/combat.rs) `equipped_defense`
