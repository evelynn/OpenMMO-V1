...
**진행**: 30개 항목 중 **13개 완료**. **M0·M1이 전부 끝났다** (SPK-1은 go).
...
| IMP-0.2 EffectiveStats | ✅ 완료 (프로토콜 v34, 13 개정 5건) | 아래 참조 |

**다음에 집을 것**: **M0와 M1이 모두 끝났다.** 체크되지 않은 첫 행은 **순번 12,
IMP-2.2 세이브 포인트**이고, 그다음은 SPK-2(창고 델타 측정) → IMP-2.3(창고) →
SPK-3 → IMP-2.4(유료 이동) → IMP-2.7 → IMP-2.8 순이다. M2만 남았다.
...
- **프로토콜이 v29 → v34로 다섯 번 올랐다.** 서버와 클라이언트를 반드시 함께 배포한다.
