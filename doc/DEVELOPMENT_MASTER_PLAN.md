...
| IMP-7.2 플레이어 봇 | ✅ 완료 (규칙 드라이버, LLM 비용 0, 프로토콜 변경 없음, 13 개정 3건) | `PENDING` |
...
| 순번 | 작업 ID | 작업 | 선행 | 크기 | 영역 | 산출물 |
|------|---------|------|------|------|------|--------|
| 40 | IMP-6.1 | **도시 서비스 NPC 배치** | 지형 베이크 | M | data/agent-client | IMP-2.2·2.3이 "공식 NPC 누구나"로 도는 것을 전용 NPC로 좁힌다. `npcs.csv` 행 + `schedule.json`의 **검증된 월드 좌표** + 프롬프트. 지형이 없어 미뤄져 있었으나 **이 환경에는 베이크가 있다** |
