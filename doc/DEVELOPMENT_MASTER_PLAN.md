...
| IMP-5.5 운영 최소선 | ✅ 완료 (health·ready·metrics, 스모크 4검사 추가, 13 개정 2건) | `PENDING` |
...
**다음에 집을 것**: **M5 39행** — 5.6(브라우저 인게임 검증). 지형이 베이크된
환경이 필요하므로 이 컨테이너에서는 착수할 수 없다 — 남은 유일한 항목이자
§0의 가장 오래된 빚이다. → 5.3(에이전트 미도달) → 5.4(사람 미도달 + 죽은 와이어) →
