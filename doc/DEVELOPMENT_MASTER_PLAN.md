...
| IMP-4.5 에이전트 동반자 계약 | ✅ 완료 (프로토콜 v45, 스키마 변경 없음, 13 개정 5건) | `PENDING` |
| IMP-4.6 제한형 매크로 | ✅ 완료 (서버 변경 0, localStorage만, 13 개정 4건) | `c182e19` |

**다음에 집을 것**: **IMP-4.4(제작)** — 도입 확정, NPC 제작 + 대장간 자가 제작
두 갈래. 그 뒤 전체 구성 갭 분석. **IMP-4.3(거점 점유)은 보류 확정**이다.
...
- **프로토콜이 v29 → v45로 열여섯 번 올랐다.** 서버와 클라이언트를 반드시 함께 배포한다.
