...
| IMP-6.1 도시 서비스 NPC | ✅ 완료 (Mira, 좌표를 지형 샘플링으로 검증, 13 개정 2건) | `591226b` |
| IMP-7.2 플레이어 봇 | ✅ 완료 (규칙 드라이버, LLM 비용 0, 프로토콜 변경 없음, 13 개정 3건) | `9d50be4` |
| IMP-7.1 채널 (월드 분할) | ✅ 완료 (프로토콜 v48, **1,200명이 4채널에 300씩**, 13 개정 2건) | `72b3b0d` |
| IMP-6.3 실소켓 부하 시험 | ✅ 완료 (**1,000 동접 편안 · 5,000은 유지 불가, 피크 1,291**, 13 개정 1건) | `f9439ec` |
| IMP-6.2 DB 백업 · 복구 | ✅ 완료 (스모크 2검사 추가, 13 개정 1건 — 프로세스 판정을 두 번 틀렸다) | `b753c43` |
| IMP-5.6a 손님 로그인 | ✅ 완료 (프로토콜 v49, 기본 꺼짐 `--allow-guest-login`, 13 개정 1건 — **IMP-5.6의 자격 증명 게이트를 없앤다**) | `8528694` |
| IMP-8.2 상위 몬스터·종족 축 | ✅ 완료 (프로토콜 v50, 몬스터 25종·계약 20개, **새 애셋 0 — 09 #25 재판정**, 13 개정 1건 + 새 항목) | `e43fb74` |
| IMP-8.1 전직 | ✅ 완료 (프로토콜 v51, `job_tier` 컬럼, **09 #28 재판정 — 게이트를 Job 레벨로**, 13 신규 항목 + 개정 1건) | `18e93f5` |
| IMP-5.5 운영 최소선 | ✅ 완료 (health·ready·metrics, 스모크 4검사 추가, 13 개정 2건) | `297eae5` |
| IMP-5.4 사람 미도달 + 죽은 와이어 | ✅ 완료 (프로토콜 v47, 메시지 4종 제거, 13 개정 2건) | `1af05fe` |
| IMP-5.3 에이전트 미도달 복구 | ✅ 완료 (창고 개폐·길드 응답·길드 운영, 13 개정 2건) | `d9b614a` |
| IMP-5.2 경계면 자동 검사 | ✅ 완료 (vitest 3건, 두 실제 드리프트로 실패 확인) | `4336f5c` |
...
**개발 진행도**: [doc/BETA_READINESS.md](BETA_READINESS.md) 기준 **88%**
(시스템 99 · 검증 82 · 콘텐츠 78 · 운영 92, 가중). 베타를 막는 것은 기능이 아니라
**검증**이다 — 45행을 만들고 브라우저에서 한 번도 보지 않았다.

**다음에 집을 것**: **IMP-5.6(브라우저 인게임 검증)의 자격 증명 게이트가 사라졌다.**
막고 있던 것은 코드가 아니라 Google 클라이언트 ID였고, 이번에 **손님 로그인**
(프로토콜 v49, `--allow-guest-login`, 기본 꺼짐)을 내면서 이름 하나로 들어갈 수
있게 됐다. 실소켓 3/3이 계정 생성부터 입장까지 완주하는 것을 확인했으므로,
다음은 브라우저로 §0의 확인 목록을 도는 IMP-5.6 본론이다.
그 밖에 남은 것은 **IMP-6.5(여행 목적지 베이크)**와 **IMP-4.3(보류 확정)**,
그리고 **애셋 2건**(노비스 메시 · 2차 전직 각성 외형 — `doc/assets/characters.md`)이다.
IMP-6.4(콘텐츠 확충)는 IMP-8.2와 IMP-8.1로 쪼갰고 둘 다 끝났다.
2차 갭 분석 결과는 [doc/GAP_ANALYSIS.md](GAP_ANALYSIS.md) §0. → 5.3(에이전트 미도달) → 5.4(사람 미도달 + 죽은 와이어) →
...
- **인게임 검증은 절반만 갚았다.** `tools/smoke-local.sh`(신규)가 기계가 판단할 수
  ...
  빌드는 통과도 실패도 거짓말이기 때문이다. **이 30개 검사는 4×4 리전을 베이크한
  환경에서 전부 통과했다**
...
- **프로토콜이 v29 → v48로 열아홉 번 올랐다.** 서버와 클라이언트를 반드시 함께 배포한다.
- ~~**경계면 거울이 검사되지 않는다**~~ — **IMP-5.2에서 갚았다.**
  `protocol-mirror.test.ts`가 `shared/src/messages.rs`·`skills.rs`를 읽어
  `networkTypes.ts`의 유니온과 `skillsStore.ts`의 표시 이름을 대조한다. 실제로
  어긋났던 두 건(`ClientMessage` 12종, `SkillId::Crafting`)으로 실패를 확인했다.
  **`agent-client`의 액션 표는 아직 이 검사 밖이다** — 그쪽은 `ClientMessage`를
  전부 덮을 필요가 없어서(디버그·인증·이동 원시 메시지는 액션이 아니다) 무엇이
  빠졌는지를 기계가 판정할 수 없다. IMP-5.3이 사람 판단으로 메운다.
- ~~**에이전트가 창고를 열 수 없고 길드 초대에 답할 수 없다**~~ — **IMP-5.3에서
  갚았다.** `storage open/close`와 `guild accept/decline/kick/rank/transfer`가
  생겼고, 잘못된 안내 문구도 고쳤다. 에이전트가 자기 길드 명단을 보게 된 것은
  덤이 아니라 전제였다(13 IMP-5.3 개정 ①).
- ~~**브라우저에서 길드장을 넘길 수 없다**~~ — **IMP-5.4에서 갚았다**(길드장 이양 버튼 + 확인, Escape 시전 취소, 죽은 메시지 4종 제거).
...
- ~~**도시 서비스 NPC가 아직 없다**~~ — **IMP-6.1에서 갚았다**(Mira, 스폰에서 8m).
  아래는 그때의 기록이다.
...
### M7 — 수용 인원과 인구 (유지보수자 결정, 2026-08-20)

| 순번 | 작업 ID | 작업 | 선행 | 크기 | 영역 | 산출물 |
|------|---------|------|------|------|------|--------|
| [x] 45 | IMP-7.1 | **채널 (월드 분할)** | IMP-6.3 | L | shared/server/client | 동시 접속 목표를 **채널당 1,000명**으로 낮추고 채널로 나눈다. **같은 DB · 월드만 분리** — 캐릭터·창고·길드·우편은 하나다. 거르는 곳은 `players_within_position` 한 곳 |
| [x] 46 | IMP-7.2 | **플레이어 봇 (파티 가능한 월드 봇)** | — | L | agent-client | 실제 유저처럼 사냥터에서 사냥하고 파티에도 들어오는 규칙 기반 봇. **LLM 비용 0**, 프로토콜 변경 없음(제약 (c)) |

---

### M6 — 베타 게이트 (신규)

M5가 "무너지지 않는가"였다면 여기는 **"사람을 들여도 되는가"**다. 기준과 실측은
[doc/BETA_READINESS.md](BETA_READINESS.md)에 있고, 아래는 그 문서가 센 남은 일을
작업 행으로 옮긴 것이다. **B-5(콘텐츠 확충)는 분량이 제품 판단이라 행으로만 세우고
수치는 그 문서에 둔다.**

| 순번 | 작업 ID | 작업 | 선행 | 크기 | 영역 | 산출물 |
|------|---------|------|------|------|------|--------|
| [x] 40 | IMP-6.1 | **도시 서비스 NPC 배치** | 지형 베이크 | M | data/agent-client | IMP-2.2·2.3이 "공식 NPC 누구나"로 도는 것을 전용 NPC로 좁힌다. `npcs.csv` 행 + `schedule.json`의 **검증된 월드 좌표** + 프롬프트. 지형이 없어 미뤄져 있었으나 **이 환경에는 베이크가 있다** |
| [x] 41 | IMP-6.2 | **DB 백업 · 복구 절차** | — | S | tools/docker | 백업 스크립트도 복구 문서도 없다. SQLite이므로 `VACUUM INTO`가 잠금 없이 스냅숏을 뜬다 |
| [x] 42 | IMP-6.3 | **실소켓 부하 시험** | — | M | tools | SPK-1은 **in-process 시뮬레이션**이다. WebSocket을 실제로 N개 열어 핸드셰이크·역압·메모리를 잰다 |
| ~~43~~ | ~~IMP-6.4~~ | ~~**콘텐츠 확충**~~ | — | L | data | **IMP-8.2 + IMP-8.1로 쪼갰다** (2026-08-20). 한 줄 안에 "놀 대상이 없는 것"(데이터)과 "성장 축이 하나뿐인 것"(프로토콜)이 섞여 있어 어느 쪽도 끝나지 않았다 |
| 44 | IMP-6.5 | **월드 추가 베이크 → 여행 목적지** | 운영 | M | data/terrain | 정착지 132곳 중 **베이크 안에 4곳**뿐이라 목적지가 2개다. 코드가 아니라 베이크 문제다 |
| [x] 45 | IMP-8.2 | **상위 몬스터와 종족 축** | — | M | data/server | 몬스터 13종·레벨 1~10 → **25종·레벨 1~20**. 새 메시 0 — `orc_boss`가 이미 쓰던 base+`scale` 수법. 09 #25(종족 축)를 재판정해 **5종**으로 채택하고 사냥 계약의 목표로 쓴다. 계약 6 → 20 |
| [x] 46 | IMP-8.1 | **전직 (노비스 → 1차 → 2차)** | IMP-8.2 | L | shared/server/client | 09 #28을 재판정한다. 기각 사유는 레벨 대역이었고, 게이트를 **Job 레벨**에 걸면 사라진다. 1차에서 **모습이 바뀌고**(클래스가 모델 경로를 정하므로 공짜) 2차에서 각성 외형이 붙는다 |
...
| [x] 35 | IMP-5.2 | **경계면 자동 검사** | — | S | client | 서버 열거형(`ClientMessage`·`SkillId`)과 클라이언트 유니온이 어긋나는지 테스트로 잡는다. 실제로 두 번 어긋났고(갭 분석 A-1/A-2) 한 번은 **CI가 이미 빨간 채로 머지되어 있었다** |
| [x] 36 | IMP-5.3 | **에이전트 미도달 기능 복구** | — | M | agent-client | 창고를 열 수 없고(`OpenStorage` 미도달) 길드 초대에 답할 수 없다(`RespondGuildInvite` 미도달). **두 액션의 설명 문구가 되지 않는 방법을 안내하고 있어** LLM이 헛수고를 반복한다. 제약 (c) 위반이기도 하다 |
| [x] 37 | IMP-5.4 | **사람 미도달 기능 복구 + 죽은 와이어 정리** | — | S | client/shared | 길드장 이양·시전 취소에 UI가 없다. `PlaceHouse`/`ModifyRoom`/`RemoveHouse`는 서버가 명시적으로 거절하는 잔재이고 `HousesInArea`는 아무도 만들지 않는다 |
| [x] 38 | IMP-5.5 | **운영 최소선 (health · 지표)** | — | S | server | HTTP 라우트가 `/api/announcements`·지형·하우징뿐이고 **health/readiness 엔드포인트가 없다.** 5,000 동접을 지향하면서 "살아 있는가"를 물을 방법이 없다 |
| 39 | IMP-5.6 | **브라우저 인게임 검증 1회 완주** | 지형 베이크 환경 + 손님 로그인(v49) | M | — | §0의 가장 오래된 빚. 32개 항목 전부 브라우저 확인 0건이다. §3 D5가 요구하는 것이므로 **DoD를 지키지 못하고 있다는 사실 자체가 항목**이다 |
