...
**진행**: 30개 항목 중 **14개 완료**. **M0·M1이 전부 끝났고**(SPK-1은 go), M2가 4/10이다.
...
| IMP-2.2 세이브 포인트 | ✅ 완료 (프로토콜 v35, DB 4컬럼, 13 개정 4건) | 아래 참조 |
| IMP-0.2 EffectiveStats | ✅ 완료 (프로토콜 v34, 13 개정 5건) | `51a15b7` |

**다음에 집을 것**: 체크되지 않은 첫 행은 **순번 13, SPK-2(창고 델타 전송 측정)**이고,
그것이 IMP-2.3(창고)의 슬롯 상한 120을 검증한다. 그다음은 SPK-3 → IMP-2.4(유료 이동) →
IMP-2.7(미니보스) → IMP-2.8(MVP 기여도) 순으로 M2가 끝난다.
