...
| IMP-4.1 길드 | ✅ 완료 (프로토콜 v43, 테이블 4개, **13 개정 4건 — 길드 하우스는 하우징에 소유권이 없어 보류**) | `(이 커밋)` |

**다음에 집을 것**: **IMP-4.2(인스턴스 던전)** → IMP-4.6(제한형 매크로) →
IMP-4.5(에이전트 동반자) → IMP-4.4(제작). **IMP-4.3(거점 점유)은 보류 확정**이고,
...
- **프로토콜이 v29 → v43으로 열네 번 올랐다.** 서버와 클라이언트를 반드시 함께 배포한다.
