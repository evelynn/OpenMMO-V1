...
**개발 진행도**: [doc/BETA_READINESS.md](BETA_READINESS.md) 기준 **84%**
(시스템 98 · 검증 82 · 콘텐츠 57 · 운영 92, 가중). 베타를 막는 것은 기능이 아니라
**검증**이다 — 40행을 만들고 브라우저에서 한 번도 보지 않았다.
...
그 밖에 남은 것은 **IMP-8.1(전직)**, **IMP-6.5(여행 목적지 베이크)**,
그리고 **IMP-4.3(보류 확정)**이다. IMP-6.4(콘텐츠 확충)는 IMP-8.2와 IMP-8.1로
쪼갰고, 앞의 것은 끝났다.
...
| 46 | IMP-8.1 | **전직 (노비스 → 1차 → 2차)** | IMP-8.2 | L | shared/server/client | 09 #28을 재판정한다. 기각 사유는 레벨 대역이었고, 게이트를 **Job 레벨**에 걸면 사라진다. 1차에서 **모습이 바뀌고**(클래스가 모델 경로를 정하므로 공짜) 2차에서 각성 외형이 붙는다 |
