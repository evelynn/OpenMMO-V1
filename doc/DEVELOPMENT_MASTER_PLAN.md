...
**진행**: 30개 항목 중 **11개 완료**, 1개 미착수로 남김. **M1이 전부 끝났다.**
...
| IMP-1.7 채팅 접두사 | ✅ 완료 (`%` 파티, `%%` 이스케이프, `$`는 자리만) | 아래 참조 |
| IMP-0.2 EffectiveStats | ⏸ 미착수 — **게이트가 아니다.** 먼저 하면 뒤 항목의 시트 표시가 공짜가 된다 | — |

**다음에 집을 것**: **M1(밸런스 축)이 전부 끝났다.** §5 표를 위에서부터 읽는 평상
규칙으로 돌아오면 체크되지 않은 첫 행은 **순번 2, SPK-1 밀집 전투 부하 스파이크**다 —
측정 전용 항목이고, 그 결과가 IMP-4.3(착수 여부)과 IMP-2.8(축소 여부)의 게이트다.
그다음은 IMP-0.2(EffectiveStats, 게이트 아님) → M2의 IMP-2.2(세이브 포인트) →
SPK-2 → IMP-2.3(창고) 순이다.
...
**아직 갚지 않은 빚**
- **인게임 검증 0건.** 완료한 8개 항목 모두 지형이 베이크되지 않은 컨테이너에서
...
- **프로토콜이 v29 → v33으로 네 번 올랐다.** 서버와 클라이언트를 반드시 함께 배포한다.
...
- **무엇을 잰다**: `server/src/game_state/tests/spawn_scale_tests.rs`와 같은 형태로
  ...
  **200명**)을 넣고 ① `tick_player_movement` 1회 소요 시간(예산 200ms),
  ② 초당 팬아웃 메시지 수와 총 바이트, ③ 브로드캐스트 `Lagged` 발생 여부,
  ④ 전투 브로드캐스트 1회의 수신자 수·직렬화 바이트, ⑤ `players`/`monsters` 쓰기 락
