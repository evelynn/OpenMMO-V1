...
**개발 진행도**: [doc/BETA_READINESS.md](BETA_READINESS.md) 기준 **71%**
(시스템 97 · 검증 62 · 콘텐츠 41 · 운영 74, 가중). 베타를 막는 것은 기능이 아니라
**검증**이다 — 39행을 만들고 브라우저에서 한 번도 보지 않았다.

**다음에 집을 것**: 유지보수자 결정 2건(IMP-7.1 채널 · IMP-7.2 봇)은 끝났다.
남은 것은 **IMP-5.6(브라우저 인게임 검증)뿐이고, 이 컨테이너에서는
착수할 수 없다.** 지형은 베이크되어 있으나 **브라우저 진입에 Google ID 토큰이
필요**하고 `GOOGLE_CLIENT_ID`도 계정도 없다(우회 경로 없음 — `AuthenticateNpc`는
에이전트 전용). 자격 증명이 있는 환경에서 §0의 확인 목록을 완주하면 M5가 닫힌다.
그 밖에 남은 것은 **IMP-4.3(보류 확정)** 하나다.
