...
**진행**: 30개 항목 중 **12개 완료**, 1개 미착수로 남김. **M1이 전부 끝났고, SPK-1은 go다.**
...
| SPK-1 밀집 전투 부하 | ✅ **go** (200명 기준 예산의 7.6%, 13 개정 1건 — 판정 채널 정정) | 아래 참조 |
| IMP-0.2 EffectiveStats | ⏸ 미착수 — **게이트가 아니다.** 먼저 하면 뒤 항목의 시트 표시가 공짜가 된다 | — |

**다음에 집을 것**: 체크되지 않은 첫 행은 **순번 3, IMP-0.2 EffectiveStats**이고,
그다음은 M2의 IMP-2.2(세이브 포인트) → SPK-2 → IMP-2.3(창고) 순이다.
